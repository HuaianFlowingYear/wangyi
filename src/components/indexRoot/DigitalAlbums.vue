    ...
    <div class="horizontal-container">
        <div class="flex justify-between items-center h-[12vw]">
            <div data-v-3b54968e="" class="flex items-center">
                <span data-v-3b54968e="" class="dark:text-[#e9ebf2] text-[#374d5b] text-[4vw] font-[800]">新歌新碟\数字专辑</span> 
            </div>
        </div>
      <div class="scroll-wrapper " ref="scroll">
        <div class="scroll-content flex w-[192vw] mx-auto overflow-hidden h-[40] scroll-content relative">   
          <ul class="scroll-item" v-for="(item, index) in shuzi" :key="index">
...
import { getDigitalAlbum} from '@/service/index'
export default {
    data () {
      return {
        shuzi:[],
    ...
    async created(){
        const [err,res] = await getDigitalAlbum({type:0})
        if (err) return console.log(err)
        console.log('数字专辑',res.data.products);
        this.shuzi = res.data.products
    },
    mounted() {
      this.init()
    },

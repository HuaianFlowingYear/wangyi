<template>
    <div class="bg-[#F4F4F4] dark:bg-[#000]">
      <div class="pb-[12vw] relative">
          <nav class="bg-[#6186AB] dark:bg-[#000] z-[999] t-0 pl-[3.9vw] pr-[6.4vw] h-[13.5vw] flex items-center justify-between fixed text-[white]">
              <div class="flex items-center">
                  ...
                  <div class="w-[42vw] h-[13.5vw] flex items-center overflow-hidden relative">
                      <div v-if="!nav_show" class="scrolling-text text-[4.2vw] text-[#fff] ml-[4.7vw] absolute textScrolling whitespace-nowrap">
                          <span>{{ name }}</span>
                      </div>
          ...
          <div class="w-[100%] dark:bg-[#000] h-[82vw] transition-all duration-20 0 relative bg-[#6186ab] pb-[5vw] pl-[3.9vw] pr-[3.4vw] pt-[13.5vw]">
              <div class="fixed dark:bg-[#000] top-0 h-[13.5vw] w-[100vw] left-0 z-[998] opacity-0"></div>
              <div class="transition-all fade-in dark:bg-[#000]" v-show="flag==true"> 
                  <section class="h-[29vw] flex pt-[2.6vw] justify-between dark:bg-[#000]">
                      <div class="w-[24vw] h-[25vw] pt-[1vw] relative">
                          ...
                          <div class="w-[67vw] pr-[12vw] dark:bg-[#000]">
              ...
              <div class="fade-in" v-show="flag==false">
                  ...
                  <BetterScroll class=" overflow-hidden">
                      <div class="flex overflow-hidden scroll-content" :style="{width:`${this.subscribers.length * 30}vw`}">
                          <div class="w-[28vw] scroll-item mr-[2.5vw]" v-for="(item,index) in subscribers" :key="index">
                              <div @click="dian(item.id)" class="w-[28vw] h-[28vw] rounded-[8px] overflow-hidden relative pt-[1vw]">
      ...
      <div class="bg-[#fff] dark:bg-[#000] w-[100vw] rounded-[15px] relative z-[1] mt-[-2.5vw] px-[3.8vw]">
          <nav class="h-[13vw] flex items-center justify-between">
              <div class="flex items-center" @click="mixin_player.replaceTracks(tracksList.map(({id})=>id))">
          ...
          <div>
            <router-link to="#">
                <div @click="mixin_player.replaceTracks(tracksList.map(({id})=>id),tracksList.map(({id})=>id)[index])" v-for="(item,index) in tracksList" :key="item.id" class="flex items-center h-[14vw]">
                        <div class="w-[4vw] text-[#bfbfbf] text-[3vw] text-center mr-[3.52vw] font-medium">
  ...
  export default {
    ...
    async created(){
        // const id = this.$route.query.personid; // 检查id的值是否正确
